...
**A base de demonstração traz empresas reais de Cuiabá** — Colatte (a vaga principal e o candidato da demonstração ao vivo), Log,Lab, Amaggi, Grupo Norte Logística e Bom Futuro — ao lado das três empresas fictícias do roteiro. O traçado cultural delas é **ilustrativo, derivado do que cada uma publica no próprio site**, e está marcado assim na tela e no código; o perfil de verdade só existe quando a amostra de colaboradores responde (R2). Nenhuma pessoa real entra: contatos são canais genéricos, e todos os candidatos são fictícios. Fontes e mapeamento tema a tema em `docs/cliente/05-seed-empresas-reais.md`.
...
- **Provedor**: Gemini (Google), pelo endpoint compatível com a OpenAI (`generativelanguage.googleapis.com/v1beta/openai`, modelo `gemini-3.6-flash` por padrão; `GEMINI_MODEL` troca). DeepSeek e Anthropic ficam como alternativas, com a mesma regra e o mesmo pacote pseudonimizado. Só liga com `IEL_AI_PROVIDER` e a chave do provedor; sem elas, tudo roda na regra fixa, sem IA paga. Atenção ao plano gratuito do Gemini: a cota é de 20 pedidos por dia por modelo — para uma demonstração, faturamento ligado no AI Studio, ou um modelo alternativo em `GEMINI_MODEL`.
...
- **Contenção**: a rota do Mind aceita 20 pedidos por minuto por IP; timeout de 20 s; qualquer falha do modelo (rede, 4xx/5xx, cota, texto fora do formato) cai para a regra fixa com aviso na resposta.
- **A devolutiva pessoal** (`/api/iel/leitura-pessoal`, fim dos dois questionários) segue a mesma regra, com um pacote menor. **Sai**: o papel (candidato/colaborador), a média por tema em número com o lado (alto/baixo) e o rótulo do tema, as frases prontas da regra fixa e `atividade`/`setor` genéricos, passados pela pseudonimização. **Não sai**: nome (a tela nem manda), nome da empresa (R5), id de candidatura ou de convite, e as respostas frase a frase. O modelo só reescreve as frases da regra; se mudar a direção, o tema, a quantidade ou virar julgamento, o texto dele é descartado e a regra fica, sem aviso — timeout de 5 s, mesmo limite de 20 pedidos por minuto, e sem chave a rota responde a regra com 200.
- **A mensagem ao candidato** (`/api/iel/mensagens`, o rascunho de WhatsApp que a analista aprova em cada etapa: convite, lembrete, currículo enviado, a empresa quer conversar, não foi desta vez, como está sendo) segue a mesma regra. **Sai**: a etapa, o texto da regra fixa com marcadores no lugar do nome, da cidade, do link e de quem assina (`[nome]`, `[cidade]`, `[link]`, `[analista]`), a atividade e o turno genéricos, o prazo em "dd/mm" e o marco (30/60/90), passados pela pseudonimização. **Não sai**: o nome da pessoa (chega à rota e fica no servidor), a cidade, o link (carrega o id da candidatura), o nome de quem assina e o nome da empresa (R5) — o corpo nem tem campo para ele. O modelo só reescreve o texto da regra; se passar de 90 palavras, perder ou dobrar um marcador, escrever um endereço à mão, citar uma empresa da base, usar palavra vedada ("garantido", "aprovado", "reprovado", "eliminado", "selecionado automaticamente", "%", "processo seletivo", "prezado", "candidato") ou trocar a etapa, o texto dele é descartado e a regra fica, sem aviso — timeout de 5 s, mesmo limite de 20 pedidos por minuto, e sem chave a rota responde a regra com 200. **Nada é enviado pela rota**: ela devolve um rascunho; a analista lê, ajusta se quiser, copia e manda pelo WhatsApp do IEL (supervisão humana).
...
- **A devolutiva pessoal não é classificação.** No fim do questionário, a pessoa recebe "o seu jeito de trabalhar" (candidato) ou "como você descreveu o seu ambiente" (colaborador): até três frases prontas, escolhidas pelas respostas dela e escritas em palavra comum, uma nuance quando ficou no meio-termo, e o aviso de que não é nota nem teste. É o que ela disse, reescrito — nunca "perfil", "tipo" ou personalidade; sem percentual, sem comparação com a empresa, e ninguém além dela vê o texto (`analysis/leitura-pessoal.ts`). Quem responde precisa receber algo de volta, e o que recebe tem de ser sobre o que respondeu, não sobre quem é.
